...
from purgefirebase import setup,checkuser,loaddata,purchase,decreasestock,increasecredit,decreasecredit
import asyncio
import requests
from cards import *
...
async def yo(ctx):
	...
	if numofyo > 0:
		...
		if syafiq in members:
			voice = await channel.connect()
			#play audio
			voice.play(discord.FFmpegPCMAudio('discordbot\yo.mp3'))
			voice.source = discord.PCMVolumeTransformer(voice.source)
			voice.source.volume = 0.1
			sleep(12)
			await ctx.send("yoe")
